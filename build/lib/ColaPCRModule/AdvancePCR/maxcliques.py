...
def graph_constructe(src_pts, tgt_pts):
    ...
    FCG = torch.clamp(1 - cross_dis ** 2 / 0.1 ** 2, min=0)  # 区间保持超过则截断函数
    FCG = FCG - torch.diag_embed(torch.diag(FCG))
    FCG[FCG < 0.99] = 0  # 相容分数小于一定阈值则直接删除
    SCG = torch.matmul(FCG, FCG) * FCG
    return SCG 


# DONE: 继续整理代码! 与之前毕设的代码合并! 形成一套体系!
def max_clique(src_pts, tgt_pts):
    """
    输入匹配对, 输出配准结果
    ...
    SCG = graph_constructe(src_pts, tgt_pts)  # (N, N) 的对称阵

    # search cliques
    # input: 对称矩阵SCG, 权重大于0的地方建边，否则不建立边

    SCG = SCG.cpu().numpy()  # 构图需要输入 list，先转为numpy
    t1 = time.perf_counter()  # TODO: 计时方式 -> 单独的示例代码
    graph = igraph.Graph.Adjacency((SCG > 0).tolist())  # 根据对称矩阵创建图(无权重，1的地方则是边)
...
def run_test():
    ...
    src_desc, tgt_desc = fpfh2(src_kpts, tgt_kpts)
    ...
    final_trans = max_clique(src_pts, tgt_pts)
    ...
    # 可视化
    putils.visualization_2phase(src_pcd, tgt_pcd, final_trans)
